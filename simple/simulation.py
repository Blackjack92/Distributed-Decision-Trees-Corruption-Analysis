...
def simulate(max_depth, number_of_corrupted_nodes, iterations, verbose=False):
    tree_combinations_generator = simple_algorithm.build_random_corrupted_tree_combinations
    ...
    corruption_validator = simple_algorithm.simple_corruption_validator
...
def main(argv):
    ...
    verbose = False
    try:
        opts, args = getopt.getopt(argv, "hd:c:i:v", ["max_depth=", "corrupted_nodes=", "iterations=", "verbose"])
    except getopt.GetoptError:
        print("analysis.py -d <max tree depth> -c <number of corrupted nodes> -i <iteration> -v <verbose>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print("analysis.py -d <max tree depth> -c <number of corrupted nodes> -i <iteration> -v <verbose>")
    ...
    # Here is the simulation executed
    simulate(max_depth, number_of_corrupted_nodes, iterations, verbose)
