...
def calculate_corruption_factor_of_corrupted_parent_nodes(corrupted_parent_nodes, max_visits, visits_per_node):
    corruption_factor = 0
    for node in corrupted_parent_nodes:
        visits = visits_per_node[node]
        corruption_factor += visits
