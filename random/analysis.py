...
def analyse(max_depth, number_of_corrupted_nodes, verbose=False):
    tree_combinations_generator = random_algorithm.build_all_corrupted_tree_combinations
    tree_combinations_generator_params = {"max_depth": max_depth,
                                          "number_of_corrupted_nodes": number_of_corrupted_nodes}
    corruption_factor_calculator = random_algorithm.calculate_corruption_factor_of_corrupted_parent_nodes
    # corruption_factor_calculator_params = {"max_visits": 100, "visits_per_node": {0: 100, 1: 0, 2: 100, 3: 0, 4: 0, 5: 50, 6: 50}}
    # corruption_factor_calculator_params = {"max_visits": 100, "visits_per_node": {0: 100, 1: 50, 2: 50, 3: 25, 4: 25, 5: 25, 6: 25}}
    corruption_factor_calculator_params = {"max_visits": 100, "visits_per_node": {0: 100, 1: 0, 2: 100, 3: 0, 4: 0, 5: 0, 6: 100}}
    corruption_validator = random_algorithm.simple_corruption_validator
...
def main(argv):
    ...
    verbose = False
    try:
        opts, args = getopt.getopt(argv, "hd:c:v", ["max_depth=", "corrupted_nodes=", "verbose"])
    except getopt.GetoptError:
        print("analysis.py -d <max tree depth> -c <number of corrupted nodes> -v <verbose>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print("analysis.py -d <max tree depth> -c <number of corrupted nodes> -v <verbose>")
    ...
    analyse(max_depth, number_of_corrupted_nodes, verbose)
