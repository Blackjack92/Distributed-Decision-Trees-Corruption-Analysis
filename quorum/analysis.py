...
def analyse(max_depth, number_of_quorums, number_of_corrupted_nodes, verbose=False):
    tree_combinations_generator = quorum_algorithm.build_all_corrupted_quorum_tree_combinations
    tree_combinations_generator_params = {"max_depth": max_depth,
                                          "number_of_quorums": number_of_quorums,
...
def main(argv):
    ...
    verbose = False
    try:
        opts, args = getopt.getopt(argv, "hd:q:c:v", ["max_depth=", "quorums=", "corrupted_nodes=", "verbose"])
    except getopt.GetoptError:
        print("analysis.py -d <max tree depth> -q <number of quorums> -c <number of corrupted nodes> -v <verbose>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print("analysis.py -d <max tree depth> -q <number of quorums> -c <number of corrupted nodes> -v <verbose>")
    ...
    print('number_of_quorums: ' + str(number_of_quorums))
    print('number_of_corrupted_nodes: ' + str(number_of_corrupted_nodes))
    print('verbose mode is on') if verbose else None
    dt_algorithm.hr()

    # Here is the analysis executed
    analyse(max_depth, number_of_quorums, number_of_corrupted_nodes, verbose)
